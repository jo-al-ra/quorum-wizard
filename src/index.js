...
if (argv.q) {
  ...
  inquirer.prompt([INITIAL_MODE]).then(async ({mode}) => {
    if (mode === 'exit') {
      info('Exiting...');
      return;
    }
...
function printInstructions(config) {
  ...
  if (isTessera(config.network.transactionManager)) {
    try {
      info(
        `A private simpleStorage contract was created with privateFor set to use Node 2's public key: ${loadTesseraPublicKey(
          config,
          2,
        )}`,
      );
      ...
      console.log(
        'cant generate private contract example because you are just using 1 node',
      );
    }
  }
}
